 ...
 */
enum abstract MobileSpriteButtonStatus(Int) from Int to Int
{
	var NORMAL = 0;
	var PRESSED = 1;
}
...
@:generic
#end
@:allow(mobile.MobileSpriteButton)
class MobileSpriteButton extends FlxSprite implements IFlxInput
{
	/**
	 * A map that's storing every active touch's ID that's pressing a button.
	 */
	public static var buttonsTouchID:Map<Int, MobileSpriteButton> = new Map();

	/**
	 * The current state of the button, either `MobileSpriteButtonStatus.NORMAL` or `MobileSpriteButtonStatus.PRESSED`.
	 */
	public var status:MobileSpriteButtonStatus;
	...
	/**
	 * Creates a new `MobileSpriteButton` object.
	 *
	...
	public function new(x:Float = 0, y:Float = 0):Void
	{
		...
		status = MobileSpriteButtonStatus.NORMAL;
		solid = false;
		immovable = true;
		#if FLX_DEBUG
	...
	public override function update(elapsed:Float):Void
	{
		...
		if (visible)
		{
			...
			if (status != MobileSpriteButtonStatus.NORMAL && (!overlapFound || (currentInput != null && currentInput.justReleased)))
			{
		...
		for (camera in cameras)
		{
			for (touch in touches)
			{
				...
				function updateTouchID():Void
				{
					...
					{
						final prevButton:Null<MobileSpriteButton> = buttonsTouchID.get(touchID);
	...
	{
		return !(status != MobileSpriteButtonStatus.NORMAL && (!check || (currentInput != null && currentInput.justReleased)));
	}
	...
	function updateStatus(newInput:IFlxInput):Void
	{
		...
		else if (status == MobileSpriteButtonStatus.NORMAL && !ignoreDownHandler)
		{
			if (newInput.pressed)
			{
				onDownHandler();
	...
	{
		status = MobileSpriteButtonStatus.PRESSED;
